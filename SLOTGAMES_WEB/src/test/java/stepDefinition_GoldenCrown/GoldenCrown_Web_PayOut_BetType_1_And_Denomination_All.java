...
public class GoldenCrown_Web_PayOut_BetType_1_And_Denomination_All {
	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Golden Crown slot game, icon to open payout table, all symbols, max amount on each symbol, back button, bet type (\\d+)\\.(\\d+) and all denomination values$")
	public void chrome_browser_valid_URL_valid_login_details_Golden_Crown_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_back_button_bet_type_and_all_denomination_values(int arg1, int arg2) throws Throwable {
		this.driver =  GoldenCrown_Web_URL_Login.getDriver();
	}

	@When("^Open the Golden Crown slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol, click on back button, select bet type as (\\d+)\\.(\\d+) & all denomination  values and check payout amount$")
	public void open_the_Golden_Crown_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_click_on_back_button_select_bet_type_as_all_denomination_values_and_check_payout_amount(int arg1, int arg2) throws Throwable {
		...
		Pattern pat1=new Pattern("E:/Sikuli Images/GoldenCrown/payout1_1.png");
		Pattern pat2=new Pattern("E:/Sikuli Images/GoldenCrown/payout1_2.png");
		Pattern pat3=new Pattern("E:/Sikuli Images/GoldenCrown/payout1_3.png");
		Pattern pat4=new Pattern("E:/Sikuli Images/GoldenCrown/payout1_4.png");
		Pattern pat5=new Pattern("E:/Sikuli Images/GoldenCrown/payout1_5.png");
		...
		System.out.println("Selected credit value is 0.01 & Comparision is successful. Test case passed");
		finder.destroy();  
		}         
		else    
		{ 
		...
		//comparing the bet value should be 0.2
	    Pattern credit11=new Pattern("E:/Sikuli Images/GoldenCrown/bet1_1.png");
		...
		System.out.println("Selected bet value is 0.2 & Comparision is successful. Test case passed");
		finder11.destroy();  
		}         
		else    
		{ 
	...
	@Then("^System should display the Payout max amount on each symbol and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination values in Golden Crown slot game on Web$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_values_in_Golden_Crown_slot_game_on_Web(int arg1, int arg2) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}